...
function sincronizarListaCanciones() {
    ...
    window.onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            listaCancionesCompartida.push({ idDoc: doc.id, ...doc.data() });
        });
        
        // Ordenamos para que las agregadas más recientemente salgan de primeras
        listaCancionesCompartida.sort((a, b) => b.fecha - a.fecha);

        // Truco de Tiempo Real: Si el modal de la lista está abierto, lo actualizamos al instante
        if (document.getElementById('modal-ver-lista-canciones').style.display === 'flex') {
...
function abrirModalAgregarCancion() {
    ...
    document.getElementById('input-cancion-artista').value = "";
...
async function guardarEnListaCompartida() {
    ...
    let artista = document.getElementById('input-cancion-artista').value.trim();
...
function renderizarListaCanciones() {
    ...
    if(listaCancionesCompartida.length === 0) {
        ...
        listaCancionesCompartida.forEach(cancion => {
            ...
            textoDiv.innerHTML = `<strong>${cancion.titulo}</strong> <br> <small style="color:#6b7280">${cancion.artista || 'Sin artista'}</small>`;
            
            // Botón opcional para eliminar de la lista (por si te equivocas o ya tocaron la canción)
...
async function eliminarCancionDeLista(idDoc) {
    if(confirm("¿Quieres quitar esta canción de la lista?")) {
