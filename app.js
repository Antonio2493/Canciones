...
async function guardarRepertorioLibre() {
    ...
    if (match) {
        ...
        if (typeof bemolesMap !== 'undefined' && bemolesMap[raizStr]) raizStr = bemolesMap[raizStr];
        let sufijo = match[2];
    ...
    // 1. CALCULAR EL TRASTE MÍNIMO PARA DESPLAZAR EL DIAPASÓN
    ...
    // 2. DIBUJAR LA LÍNEA SUPERIOR (Gruesa si es cejuela abierta, delgada si es traste desplazado)
    ...
    // Si el acorde está más abajo en el mástil, mostramos el número de traste a la izquierda
    if (baseFret > 1) {
    ...
    pos.forEach((traste, cuerda) => {
        ...
        if (traste === -1) { 
            ...
            // Solo muestra 'o' si estamos en la parte superior abierta
            if (baseFret === 1) {
