...
function abrirModalListaPersonalizada() {
    document.getElementById('modal-lista-personalizada').style.display = 'flex';
    cancionesTemporales = [];
    document.getElementById('input-nombre-lista').value = "";
    document.getElementById('input-custom-titulo').value = "";
    document.getElementById('input-custom-tono').value = "";
    actualizarVistaItemsTemporales();
}

function cerrarModalListaPersonalizada() {
    document.getElementById('modal-lista-personalizada').style.display = 'none';
}

function agregarCancionTemporal() {
    let titulo = document.getElementById('input-custom-titulo').value.trim();
    let tono = document.getElementById('input-custom-tono').value.trim();

    if (!titulo) {
        alert("Por favor escribe al menos el título de la canción.");
        return;
    }

    cancionesTemporales.push({ titulo, tono: tono || "N/A" });
    
    // Limpiar inputs para la siguiente
    document.getElementById('input-custom-titulo').value = "";
    document.getElementById('input-custom-tono').value = "";
    document.getElementById('input-custom-titulo').focus();

    actualizarVistaItemsTemporales();
}

function actualizarVistaItemsTemporales() {
    let contenedor = document.getElementById('contenedor-items-temporales');
    if (cancionesTemporales.length === 0) {
        contenedor.innerHTML = "<p style='text-align:center; color: var(--text-muted); font-size: 0.9rem;'>No hay canciones agregadas todavía.</p>";
        return;
    }

    contenedor.innerHTML = "";
    cancionesTemporales.forEach((item, index) => {
        ...
        div.innerHTML = `
            <span><strong>${index + 1}.</strong> ${item.titulo} <span style="color: var(--text-muted);">(${item.tono})</span></span>
            <button onclick="eliminarItemTemporal(${index})" style="background: none; border: none; color: #ef4444; cursor: pointer; font-weight: bold;">✕</button>
        `;
...
async function guardarListaPersonalizadaEnFirestore() {
    let nombreLista = document.getElementById('input-nombre-lista').value.trim();
    if (!nombreLista) {
        alert("Por favor asígnale un nombre a la lista.");
        return;
    }

    if (cancionesTemporales.length === 0) {
        alert("Agrega al menos una canción a la lista antes de guardar.");
        return;
    }

    try {
        await window.addDoc(window.collection(window.db, "listas_personalizadas"), {
            nombre: nombreLista,
            canciones: cancionesTemporales,
            fechaCreacion: new Date().toLocaleDateString()
        });
        alert("¡Lista personalizada guardada exitosamente en Firestore!");
        cerrarModalListaPersonalizada();
    } catch (error) {
        console.error("Error al guardar la lista personalizada:", error);
